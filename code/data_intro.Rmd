...
cow<-
  read_csv("./external_data/cow/COW country codes.csv") %>%
  distinct()
...
#Generate a list of existing country-year pairs for the 1970s (a universe of all possible diplomatic relations between US and other countries).
#states_y<-
#  read_csv("../external_data/cow/states2016.csv") %>%
#  select(stateabb, ccode, statenme, styear, endyear) %>%
#  mutate

#Note that the dataframe below does NOT include the US but only all potential US countries who existed in the 1970s. It would have been better to generate a list of all possible country-year pairs for 1973-1979 since a cable written in the 1970s could discuss (and tagged with) a country that had existed before the 1970s. However, for comparability with other datasets, we focus on the universe of potential diplomatic partners of the U.S., that is, countries that existed in the 1970s.  
...
#Yearly cable traffic by country-year, including 0 cables by some countries on the COW list. Note that this does NOT include the cables tagged with US, due to the dataframe country_univ_70s. 
cable_n_country_year<-
  cable_n_country_date %>%
  mutate(year=lubridate::year(date)) %>%
  group_by(year, cow_ccode, country_name) %>%
  summarise(n_y = sum(n)) %>%
  right_join(country_univ_70s, by=c("year", "cow_ccode"))%>%
  filter(1972<year & year<1980) %>%
  select(-country_name) %>%
  mutate(n_y= replace(n_y, is.na(n_y), 0)) %>%
  ungroup %>%
  mutate(total_n_y = sum(n_y))
```
...
##Summary statistics of cable traffic by country
```{r, results="hide",message=FALSE}
setwd("/Users/clarahsuong/chronos_data_intro")

stargazer(as.data.frame(cable_n_country_year)[c("year", "cow_ccode", "n_y")], 
          type = "html", 
...
```
##Country-years tagged with most cables
```{r}
setwd("/Users/clarahsuong/chronos_data_intro")
...
#  ungroup %>% 
  mutate(#total_n_y = sum(n_y),
         rel.freq = paste0(round(100 * n_y/total_n_y, 0), "%")) %>%
  arrange(desc(n_y)) %>%
  top_n(10) #%>%
#  select(c(year, cow_statename, n_y, rel.freq)) #%>%
#  as_label(var.label=c("Year","Tagged Country", "Number of Cables", "Relative Frequency"))%>%
#  set_caption("Table: Country-Years with Most Cables") %>%
#  htmlTable(rnames = FALSE)
#table_country_year_top10  

stargazer(table_country_year_top10[c("year", "cow_statename", "n_y","rel.freq")],
          summary = FALSE,
          rownames = FALSE,
          type = "html", 
...
##Countries tagged with most cables
```{r}
setwd("/Users/clarahsuong/chronos_data_intro")
...
stargazer(table_country_top10[c("cow_statename", "n_c","rel.freq")],
          ...
          rownames = FALSE,
          type = "html", 
          title="Countries Tagged in Most Cables", 
          digits=1, 
          out="./data_analysis_output/top10_cable_n_country.html",
          covariate.labels=c("Tagged Country", "Number of Cables", "Relative Frequency")
          )
```
##Countries tagged with least cables
```{r}
setwd("/Users/clarahsuong/chronos_data_intro")
...
stargazer(table_country_bottom10[c("cow_statename", "n_c","rel.freq")],
          ...
          rownames = FALSE,
          type = "html", 
          title="Countries Tagged in Least Cables", 
          digits=1, 
...
#  filter(Country=="United States of America") %>% 
  ...
  right_join(country_univ_70s, by=c("year", "cow_ccode")) %>% #Note the missing values for Destination but not cow_ccode.
...
##Compare the ambassador-country-year datasets
```{r}
...
###Correlation plots
```{r}
M<-cor(us_dip_rep_m_cable_n_vis, use = "complete.obs")
M
corrplot(M, method = "ellipse")
#corrplot.mixed(M)

res1 <- cor.mtest(us_dip_rep_m_cable_n_vis, conf.level = .95)
res2 <- cor.mtest(us_dip_rep_m_cable_n_vis, conf.level = .99)
...
pooling1 <- pglm(n_y ~ dip_rep_m
              #+dip_focus
              +bi_PRE+bi_SOS, data=us_dip_rep_m_cable_n_vis, index=c("cow_ccode", "year"), family = negbin, model="pooling"#, method = "nr"
              )
summary(pooling1)

pooling2 <- pglm(n_y ~ #dip_rep_m+
              dip_focus
              +bi_PRE+bi_SOS, data=us_dip_rep_m_cable_n_vis, index=c("cow_ccode", "year"), family = negbin, model="pooling"#, method = "nr"
              )
summary(pooling2)

fixed1 <- pglm(n_y ~ dip_rep_m
              #+dip_focus
              +bi_PRE+bi_SOS, data=us_dip_rep_m_cable_n_vis, index=c("cow_ccode", "year"), family = negbin, model="within", method = "nr"
              )
summary(fixed1)

fixed2 <- pglm(n_y ~ #dip_rep_m+
                dip_focus
              +bi_PRE+bi_SOS, data=us_dip_rep_m_cable_n_vis, index=c("cow_ccode", "year"), family = negbin, model="within", method = "nr"
              )
summary(fixed2)

#random <- pglm(n_y ~ dip_rep_m+dip_focus+bi_PRE+bi_SOS, data=us_dip_rep_m_cable_n_vis, index=c("cow_ccode", "year"), family = negbin, model="random"#, method = "nr"
#               )
```
...
glm.nb_irep1 <- glm.nb(n_y~dip_irep, data = cable_n_irep_app)

glm.nb_amb <- glm.nb(n_y~ambassador, data = cable_n_irep_app)

glm.nb_politicalappointee <- glm.nb(n_y~politicalappointee, data = cable_n_irep_app)

glm.nb_good <- glm.nb(n_y~good, data = cable_n_irep_app)

glm.nb_bad <- glm.nb(n_y~bad, data = cable_n_irep_app)
...
glm.nb_pre2 <- glm.nb(n_y~bi_PRE+mil_ratio+USmilaid+allies+USdefense+USdefense_EUR+UStrade+energypc+USalign+UNpart+mpower+FOR+CAR+demo+bi_L1_PRE+EUR+AFR+MEA+SCA+EAP, data = cable_n_vis_country_y)
#Cluster by cowid

glm.nb_sos2 <- glm.nb(n_y~bi_SOS+mil_ratio+USmilaid+allies+USdefense+USdefense_EUR+UStrade+energypc+USalign+UNpart+mpower+FOR+CAR+demo+bi_L1_PRE+EUR+AFR+MEA+SCA+EAP, data = cable_n_vis_country_y)
#Cluster by cowid

glm.nb_pre3 <- glm.nb(n_y~bi_PRE+mil_ratio+USmilaid+allies+USdefense+USdefense_EUR+UStrade+energypc+USalign+UNpart+mpower+FOR+CAR+demo+bi_L14_SOS+bi_L14_PRE+bi_L58_SOS+bi_L58_PRE+EUR+AFR+MEA+SCA+EAP, data = cable_n_vis_country_y)
#Cluster by cowid

glm.nb_sos3 <- glm.nb(n_y~bi_SOS+mil_ratio+USmilaid+allies+USdefense+USdefense_EUR+UStrade+energypc+USalign+UNpart+mpower+FOR+CAR+demo+bi_L14_SOS+bi_L14_PRE+bi_L58_SOS+bi_L58_PRE+EUR+AFR+MEA+SCA+EAP, data = cable_n_vis_country_y)
#Cluster by cowid
...
glm.nb_pre1 <- glm.nb(n_y~bi_PRE, data = cable_n_vis_country_y)

glm.nb_pre1 <- glm.nb(n_y~bi_PRE, data = cable_n_vis_country_y)
glm.nb_sos1 <- glm.nb(n_y~bi_SOS, data = cable_n_vis_country_y)
