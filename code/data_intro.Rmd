...
#it also includes countries non-existent in the 1970s as a potential diplomatic partner (e.g. Croatia with country code 344 and Bosnia and Herzegovina with country code 346).
...
stargazer(as.data.frame(cable_n_country_year)[c("year", "cow_ccode", "n_c_y")], 
          type = "html", 
...
```
##Country-years with least cables
```{r}
setwd("/Users/clarahsuong/chronos_data_intro")
...
stargazer(table_country_year_bottom10[c("year", "cow_statename", "n_c_y","rel.freq")],
          ...
          rownames = FALSE,
          type = "html", 
          title="Country-Years with Least Cables", 
          digits=1, 
          out="./data_analysis_output/bottom10_cable_n_country_year.html",
          covariate.labels=c("Year","Tagged Country", "Number of Cables", "Relative Frequency"))
```

##Countries most tagged in cables
```{r}
setwd("/Users/clarahsuong/chronos_data_intro")
...
stargazer(table_country_top10[c("cow_statename", "n_c","rel.freq")],
          ...
          rownames = FALSE,
          type = "html", 
          title="Countries Most Tagged in Cables", 
          digits=1, 
...
##Countries least tagged in cables
```{r}
setwd("/Users/clarahsuong/chronos_data_intro")
...
stargazer(table_country_bottom10[c("cow_statename", "n_c","rel.freq")],
          ...
          rownames = FALSE,
          type = "html", 
          title="Countries Least Tagged in Cables", 
          digits=1, 
...
pooling1 <- pglm(n_c_y ~ dip_rep_m
              #+dip_focus
              +bi_PRE+bi_SOS, data=us_dip_rep_m_cable_n_vis, index=c("cow_ccode", "year"), family = negbin, model="pooling"#, method = "nr"
              )
summary(pooling1)

pooling2 <- pglm(n_c_y ~ #dip_rep_m+
              dip_focus
              +bi_PRE+bi_SOS, data=us_dip_rep_m_cable_n_vis, index=c("cow_ccode", "year"), family = negbin, model="pooling"#, method = "nr"
              )
summary(pooling2)

fixed1 <- pglm(n_c_y ~ dip_rep_m
              #+dip_focus
              +bi_PRE+bi_SOS, data=us_dip_rep_m_cable_n_vis, index=c("cow_ccode", "year"), family = negbin, model="within", method = "nr"
              )
summary(fixed1)

fixed2 <- pglm(n_c_y ~ #dip_rep_m+
                dip_focus
              +bi_PRE+bi_SOS, data=us_dip_rep_m_cable_n_vis, index=c("cow_ccode", "year"), family = negbin, model="within", method = "nr"
              )
summary(fixed2)

#random <- pglm(n_c_y ~ dip_rep_m+dip_focus+bi_PRE+bi_SOS, data=us_dip_rep_m_cable_n_vis, index=c("cow_ccode", "year"), family = negbin, model="random"#, method = "nr"
#               )
```
