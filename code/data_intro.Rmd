...
#Generate lists of all (old and existing) countries according to COW. Note that this includes the US.
states<-
...
#Generate lists existing country-years for the 1970s. Note that the universe of countries would differs by year. Note that this also includes the US.
states_70s<-
  read_csv("/Users/clarahsuong/chronos_data_intro/external_data/cow/states2016.csv") %>%
  ...
  left_join(states, by="cow_ccode") %>%#Attach COW state names and abbreviations
  distinct()
...
#Note: we cannot automatically complete the data frame with missing combinations of data (function complete) because we need to account for the difference in countries' birth date. 
```
...
country_code_tag<-
  ...
  inner_join(states, by="cow_ccode") %>% #Append COW state names
...
tags<-
  tbl(mydb, 'tags') %>% 
  dplyr::select(id, tag, category) %>% collect() #Most of the tags missing category don't seem to be country names.
#tags[is.na(tags$category),]
...
cable_n_country_date<-
  ...
  group_by(.dots=c("cow_ccode", 
                   ...
                   "date")) %>%
  tally() 
#save(cable_n_country_date, file = "/Users/clarahsuong/chronos_data_intro/data/cable_n_country_date.RData")
#load("./data/cable_n_country_date.RData") #Note that this includes neither all dates nor all countries on the COW list.
...
#Yearly cable traffic by country-year, including 0 cables by some countries that existed in the 1970s. Note that this does include the cables tagged with US, due to the dataframe states_70s. 
...
#Yearly cable traffic by country-year, including 0 cables by some countries that have ever existed. Note that this does include the cables tagged with US. 
cable_n_country_year<-
  country_tag_doc2 %>%
  group_by(year, cow_ccode, cow_statename, cow_stateabb) %>%
  tally() %>%
  right_join(states, by=c("year", "cow_ccode", "cow_stateabb","cow_statename")) %>%
  rename(n_c_y=n) %>%
  mutate(n_c_y= replace(n_c_y, is.na(n_c_y), 0)) %>%
  ungroup %>%
  mutate(total_n = sum(n_c_y)) %>%
...
#Yearly cable traffic by country-year, including 0 cables by some countries that existed anytime before 2016.  
#cable_n_country_year<-
#  cable_n_country_date %>%
#  mutate(year=lubridate::year(date)) %>%
...
#Cable traffic by country, including 0 cables by some countries that existed in the 1970s. Note that this does include the cables tagged with US, due to the dataframe states_70s. 
cable_n_country<-
  country_tag_doc2 %>%
  group_by(cow_ccode, cow_statename, cow_stateabb) %>%
  tally() %>%
  right_join(states, by=c("cow_ccode", "cow_stateabb","cow_statename")) %>%
  rename(n_c=n) %>%
...
table_country_year_top30<- 
  cable_n_70s_country_year %>%
  mutate(rel.freq = paste0(round(100 * n_c_y/total_n, 0), "%")) %>%
  arrange(desc(n_c_y)) %>%
  top_n(n = 30, wt = n_c_y) 

stargazer(table_country_year_top30[c("year", "cow_statename", "n_c_y","rel.freq")],
          summary = FALSE,
          rownames = FALSE,
          type = "text", 
          title="Country-Years with Most Cables", 
          digits=1, 
          out="./data_analysis_output/top30_cable_n_country_year.txt",
...
stargazer(table_country_year_top30[c("year", "cow_statename", "n_c_y","rel.freq")],
          ...
          title="Country-Years with Most Cables", 
          digits=1, 
          out="./data_analysis_output/top30_cable_n_country_year.html",
          covariate.labels=c("Year","Tagged Country", "Number of Cables", "Relative Frequency"))
```
...
stargazer(table_country_year_bottom30[c("year", "cow_statename", "n_c_y","rel.freq")],
          summary = FALSE,
          rownames = FALSE,
          type = "text", 
          title="Country-Years with Fewest Cables", 
          digits=1, 
          out="./data_analysis_output/bottom30_cable_n_country_year.txt",
          covariate.labels=c("Year","Tagged Country", "Number of Cables", "Relative Frequency"))
...
stargazer(table_country_year_bottom30[c("year", "cow_statename", "n_c_y","rel.freq")],
          ...
          title="Country-Years with Fewest Cables", 
          digits=1, 
          out="./data_analysis_output/bottom30_cable_n_country_year.html",
          covariate.labels=c("Year","Tagged Country", "Number of Cables", "Relative Frequency"))
```
...
stargazer(table_country_top30[c("cow_statename", "n_c","rel.freq")],
          summary = FALSE,
          rownames = FALSE,
          type = "text", 
          title="Countries Most Frequently Tagged in Cables", 
          digits=1, 
          out="./data_analysis_output/top30_cable_n_country.txt",
...
stargazer(table_country_top30[c("cow_statename", "n_c","rel.freq")],
          ...
          title="Countries Most Frequently Tagged in Cables", 
          digits=1, 
          out="./data_analysis_output/top30_cable_n_country.html",
          covariate.labels=c("Country", "Number of Cables", "Relative Frequency"))
```
...
stargazer(table_country_bottom30[c("cow_statename", "n_c","rel.freq")],
          summary = FALSE,
          rownames = FALSE,
          type = "text", 
          title="Countries Least Frequently Tagged in Cables", 
          digits=1, 
          out="./data_analysis_output/bottom30_cable_n_country.txt",
          covariate.labels=c("Country", "Number of Cables", "Relative Frequency"))
...
stargazer(table_country_bottom30[c("cow_statename", "n_c","rel.freq")],
          ...
          title="Countries Least Frequently Tagged in Cables", 
          digits=1, 
          out="./data_analysis_output/bottom30_cable_n_country.html",
          covariate.labels=c("Country", "Number of Cables", "Relative Frequency"))
```
