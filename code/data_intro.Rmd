...
library(sjlabelled) #For creating Word-compatible tables
...
countries<-
  tbl(mydb, 'countries') %>% 
  collect() 
#Note that there is no tag for "South Vietnam" but tag "VM" (id: 557) for "Vietnam" according to this table. The 1976 TAGS manual lists VN (tag_id 1976) for "Viet-Nam (North)"and VS (tag_id 1973) for "Viet-Nam (South)". There is also no tag_id for Serbia, Montenegro, Croatia, Azerbaijan. 
#It's probably better to not include the list of countries with 0 cables in the paper because these may be simply missing due to errors in the metadata. 
#US is included.
...
#Generate a dataframe for all (historic and existing) countries according to COW. Note that this includes the US.
all_states<-
...
#Generate a dataframe for all (historic and existing) countries for years 1973-79. Note that this includes the US.
all_states_70s<-
  read_csv("/Users/clarahsuong/chronos_data_intro/external_data/cow/states2016.csv") %>%
  dplyr::select("stateabb","ccode","statenme") %>%
  #filter(!ccode==2) %>% #Leave out the US
  mutate(statenme=as.character(statenme))%>%
  rename(cow_ccode=ccode,
         cow_stateabb=stateabb, 
         cow_statename=statenme) %>%   
  distinct() %>% #There are duplicates. e.g. countries that existed, disappeared, and then re-appeared. 
...
states_70s<-
  read_csv("/Users/clarahsuong/chronos_data_intro/external_data/cow/system2016.csv") %>%
...
country_code_tag<-
  ...
  inner_join(all_states, by="cow_ccode") %>% #Include state names from the list of all states that have ever existed.
...
#                   "date")) %>%
#  tally() 
#save(cable_n_country_day, file = "/Users/clarahsuong/chronos_data_intro/data/cable_n_country_day.RData")
...
#Yearly cable traffic by state-year, including 0 cables by some countries that did not exist in the 1970s. Note that this does include the cables tagged with US. 
#cable_n_all_states_year<-
#  country_tag_doc2 %>%
#  group_by(year, cow_ccode, cow_statename, cow_stateabb) %>%
#  tally() %>%
#  right_join(all_states_70s, by=c("year", "cow_ccode", "cow_stateabb","cow_statename")) %>%
#  rename(n_c_y=n) %>%
#  mutate(n_c_y= replace(n_c_y, is.na(n_c_y), 0)) %>%
#  ungroup %>%
#  mutate(total_n = sum(n_c_y)) %>%
#  arrange(year, cow_ccode)
#save(cable_n_all_states_year, file = "/Users/clarahsuong/chronos_data_intro/data/cable_n_all_states_year.RData")
load("./data/cable_n_all_states_year.RData")

#Cable traffic by state, including 0 cables by some countries that did not exist in the 1970s. Note that this does include the cables tagged with US. 
#cable_n_all_states<-
#  country_tag_doc2 %>%
...
#  arrange(desc(n_c))
#save(cable_n_all_states, file = "/Users/clarahsuong/chronos_data_intro/data/cable_n_all_states.RData")
load("./data/cable_n_all_states.RData")

#Yearly cable traffic by state-year, excluding 0 cables by some countries that did not exist in the 1970s. Note that this does include the cables tagged with US. 
#cable_n_states_70s_year<-
#  country_tag_doc2 %>%
#  group_by(year, cow_ccode, cow_statename, cow_stateabb) %>%
#  tally() %>%
#  right_join(states_70s, by=c("year", "cow_ccode", "cow_stateabb","cow_statename")) %>%
#  rename(n_c_y=n) %>%
#  mutate(n_c_y= replace(n_c_y, is.na(n_c_y), 0)) %>%
#  ungroup %>%
#  mutate(total_n = sum(n_c_y)) %>%
#  arrange(year, cow_ccode)
#save(cable_n_states_70s_year, file = "/Users/clarahsuong/chronos_data_intro/data/cable_n_states_70s_year.RData")
...
#  mutate(total_n = sum(n_c)) %>%
#  arrange(desc(n_c))
#save(states_70s, file = "/Users/clarahsuong/chronos_data_intro/data/cable_n_states_70s.RData")
...
stargazer(as.data.frame(cable_n_all_states_year)[c("year", "cow_ccode", "n_c_y")], 
          type = "text", 
          title="Summary Statistics of Cable Traffic by Country-Year (Incl. Historic Countries)", 
          digits=1, 
          out="./data_analysis_output/desc_cable_n_all_states_year.txt",
          covariate.labels=c("Year", "COW Codes of Countries Tagged in Cables", "Cable Traffic for Each Country-Year"))
...
stargazer(as.data.frame(cable_n_states_70s_year)[c("year", "cow_ccode", "n_c_y")], 
          type = "text", 
          title="Summary Statistics of Cable Traffic by Country-Year (Only Existing Countries)", 
          digits=1, 
...
stargazer(table_country_year_top20[c("year", "cow_statename", "n_c_y","rel.freq")],
          summary = FALSE,
          rownames = FALSE,
          type = "text", 
          title="Non-U.S. Country-Years with Most Cables", 
          digits=1, 
          out="./data_analysis_output/top20_cable_n_country_year.txt",
...
stargazer(table_country_year_top20[c("year", "cow_statename", "n_c_y","rel.freq")],
          ...
          title="Non-U.S. Country-Years with Most Cables", 
          digits=1, 
          out="./data_analysis_output/top20_cable_n_country_year.html",
          covariate.labels=c("Year","Tagged Country", "Number of Cables", "Relative Frequency"))
```
...
stargazer(table_country_year_bottom20[c("year", "cow_statename", "n_c_y","rel.freq")],
          summary = FALSE,
          rownames = FALSE,
          type = "text", 
          title="Non-U.S. Country-Years with Fewest Cables", 
          digits=1, 
          out="./data_analysis_output/bottom20_cable_n_country_year.txt",
          covariate.labels=c("Year","Tagged Country", "Number of Cables", "Relative Frequency"))
...
stargazer(table_country_year_bottom20[c("year", "cow_statename", "n_c_y","rel.freq")],
          ...
          title="Non-U.S. Country-Years with Fewest Cables", 
          digits=1, 
          out="./data_analysis_output/bottom20_cable_n_country_year.html",
          covariate.labels=c("Year","Tagged Country", "Number of Cables", "Relative Frequency"))
```
...
stargazer(table_country_top20[c("cow_statename", "n_c","rel.freq")],
          summary = FALSE,
          rownames = FALSE,
          type = "text", 
          title="Non-U.S. Countries Most Frequently Tagged in Cables", 
          digits=1, 
          out="./data_analysis_output/top20_cable_n_country.txt",
...
stargazer(table_country_top20[c("cow_statename", "n_c","rel.freq")],
          ...
          title="Non-U.S. Countries Most Frequently Tagged in Cables", 
          digits=1, 
          out="./data_analysis_output/top20_cable_n_country.html",
          covariate.labels=c("Country", "Number of Cables", "Relative Frequency"))
```
...
stargazer(table_country_bottom20[c("cow_statename", "n_c","rel.freq")],
          summary = FALSE,
          rownames = FALSE,
          type = "text", 
          title="Non-U.S. Countries Least Frequently Tagged in Cables", 
          digits=1, 
          out="./data_analysis_output/bottom20_cable_n_country.txt",
          covariate.labels=c("Country", "Number of Cables", "Relative Frequency"))
...
stargazer(table_country_bottom20[c("cow_statename", "n_c","rel.freq")],
          ...
          title="Non-U.S. Countries Least Frequently Tagged in Cables", 
          digits=1, 
          out="./data_analysis_output/bottom20_cable_n_country.html",
          covariate.labels=c("Country", "Number of Cables", "Relative Frequency"))
```
...
#  read_excel("../external_data/moyeretal2016/Diplomatic_Exchange_V3.16.16.xlsx") %>% #Non-compatible, public version
  ...
  mutate(cow_ccode=countrycode(Destination, 'country.name', 'cown')) %>% #us_dip_rep[is.na(us_dip_rep$cow_ccode),] COW country code for German Federal Republic is wrong and should be 260, not 255. This is an error in the R package countrycode.
...
us_dip_rep_app <-  
  ...
  summarise(dip_rep_a=max(dip_rep_a),
...
###Check for overdispersion of the DV
```{r}
is.pbalanced(us_dip_c_y)

plot(table(us_dip_c_y$n_c_y))
...
plot(clog(n_c_y) ~ dip_rep_m, data = us_dip_c_y)
plot(clog(n_c_y) ~ dip_focus, data = us_dip_c_y)
plot(clog(n_c_y) ~ dip_rep_a, data = us_dip_c_y)
plot(clog(n_c_y)~ politicalappointee_year, data = us_dip_c_y)
plot(clog(n_c_y) ~ performanceMU_year, data = us_dip_c_y)
plot(clog(n_c_y) ~ ambassador_year, data = us_dip_c_y)
plot(clog(n_c_y) ~ bi_PRE, data = us_dip_c_y)
plot(clog(n_c_y) ~ bi_SOS, data = us_dip_c_y)
plot(clog(n_c_y) ~ cfac(evt_n_c_y), data = us_dip_c_y)

glm.nb_politicalappointee <- MASS::glm.nb(n_c_y~politicalappointee_year+lag.n_c_y, data = us_dip_c_y)

coeftest(glm.nb_politicalappointee, vcov = sandwich)
```
...
m_rep_m2_app=summary(fixed_rep_m2_app)
m_rep_m2_app

fixed_rep_m2_vis <- pglm(n_c_y~
                   dip_rep_m+
                   dip_focus+
                   #dip_rep_a+
                   #politicalappointee_year+
...
m_rep_m2_vis=summary(fixed_rep_m2_vis)
m_rep_m2_vis
