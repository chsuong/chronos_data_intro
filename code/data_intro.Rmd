...
#Note that this does NOT for the US.
country_univ_70s<-
...
#Yearly cable traffic by country-year, including 0 cables by some countries on the COW list
cable_n_country_year<-
  ...
  select(-country_name) %>%
  mutate(y_n= replace(y_n, is.na(y_n), 0)) 
```
...
stargazer(as.data.frame(cable_n_country_year), 
          type = "html", 
          title="Summary Statistics of Yearly Cable Traffic", 
          digits=1, 
          out="./data_analysis_output/desc_cable_n_country_year.html",
          covariate.labels=c("Year", "COW Codes of the Country Tagged by Cables", "Cable Traffic for Each Country-Year")
          )
```
...
pooling1 <- pglm(y_n ~ dip_rep_m
              #+dip_focus
              +bi_PRE+bi_SOS, data=us_dip_rep_m_cable_n_vis, index=c("cow_ccode", "year"), family = negbin, model="pooling"#, method = "nr"
              )
summary(pooling1)

pooling2 <- pglm(y_n ~ #dip_rep_m+
              dip_focus
              +bi_PRE+bi_SOS, data=us_dip_rep_m_cable_n_vis, index=c("cow_ccode", "year"), family = negbin, model="pooling"#, method = "nr"
              )
summary(pooling2)

fixed1 <- pglm(y_n ~ dip_rep_m
              #+dip_focus
              +bi_PRE+bi_SOS, data=us_dip_rep_m_cable_n_vis, index=c("cow_ccode", "year"), family = negbin, model="within", method = "nr"
              )
summary(fixed1)

fixed2 <- pglm(y_n ~ #dip_rep_m+
                dip_focus
              +bi_PRE+bi_SOS, data=us_dip_rep_m_cable_n_vis, index=c("cow_ccode", "year"), family = negbin, model="within", method = "nr"
              )
summary(fixed2)

#random <- pglm(y_n ~ dip_rep_m+dip_focus+bi_PRE+bi_SOS, data=us_dip_rep_m_cable_n_vis, index=c("cow_ccode", "year"), family = negbin, model="random"#, method = "nr"
#               )
```
