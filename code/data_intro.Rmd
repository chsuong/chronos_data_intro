...
#Connecting to and exploring the databases
...
db_doc_no_date <- function(mydb) {
  ...
  return(c(mydb,nrow(docs), range(docs$date, na.rm = TRUE)))
}

#
#db_doc_no_date('declassification_cables')
db_doc_no_date('declassification_ddrs')
db_doc_no_date('declassification_frus')
db_doc_no_date('declassification_kissinger')
db_doc_no_date('declassification_pdb')
db_doc_no_date('declassification_clinton') 
db_doc_no_date('declassification_cabinet')
db_doc_no_date('declassification_cpdoc')
```
#Frequency tables for declassification_cables
```{r}
driver = dbDriver("MySQL")
connection = dbConnect(driver,host='history-lab.org', password='XreadF403', user='de_reader')
mydb = dbConnect(driver,host='history-lab.org', password='XreadF403', user='de_reader', 
...
ns<-cbind(C1, C2)
colnames(ns) <- c("Variable","N")

htmlTable(ns,  
          ctable=c("solid", "double"),
          caption="Number of Cables with Non-Missing Values")
...
stargazer(table_cable_n_year[c("year","n", "rel.freq")],
          summary = FALSE,
          rownames = FALSE,
          type = "text", 
          title="Number of Cables By Year", 
          digits=1, 
          out="./data_analysis_output/table_cable_n_year.txt",
...
stargazer(table_cable_n_year[c("year","n", "rel.freq")],
          ...
          title="Number of Cables By Year", 
          digits=1, 
          out="./data_analysis_output/table_cable_n_year.html",
          covariate.labels=c("Year","Number of Cables", "Relative Frequency")
          )
...
stargazer(table_state_year_top20[c("year", "cow_statename", "n_c_y","rel.freq")],
          ...
          rownames = FALSE,
          type = "text", 
          title="Non-U.S. Country-Years with Highest Tag Traffic", 
          digits=1, 
...
stargazer(table_state_year_bottom10[c("year", "cow_statename", "n_c_y","rel.freq")],
          ...
          rownames = FALSE,
          type = "text", 
          title="Non-U.S. Country-Years with Lowest Tag Traffic", 
          digits=2, 
...
##Plot of tag traffic of 10 countries
```{r}
...
png("./data_analysis_output/key10.png", width = 800, height = 600)
...
stargazer(russia_tag_network,
          ...
          covariate.labels=c("Year", "Number of Cables Tagged<br>with the U.S.S.R.", "Number of Cables Sent by/to<br>the U.S. Embassy in Moscow")
          )
...
stargazer(russia_tag_network,
          ...
          covariate.labels=c("Year", "Number of Cables Tagged with the U.S.S.R.", "Number of Cables Sent by/to the U.S. Embassy in Moscow")
          )
```
#Iran
```{r}
...
us_dip_rep_cow<-read_csv("./external_data/cow/Diplomatic_Exchange_2006v1.csv")
```
...
stargazer(as.data.frame(us_dip_c_y)[c("year","cow_ccode",
                                      ...
                                      "evt_n_c_y")], 
          type = "text", 
          title="Summary Statistics of Variables on U.S. Diplomacy (Unit: Country-Year)", 
          digits=1, 
          out="./data_analysis_output/desc_us_dip_states_70s_year.txt",
          covariate.labels=c("Year", "COW Country Codes", 
                             "Tag Traffic",
                            "U.S. Diplomatic Representation","U.S. Diplomatic Focus",
                            "U.S. Ambassador Is a Political Appointee",
                            "Number of U.S. Presidential Visits",
                            "Number of U.S. Diplomatic Events")
          )
...
stargazer(as.data.frame(us_dip_c_y)[c("year","cow_ccode",
          ...
          type = "html", 
          title="Summary Statistics of Variables on U.S. Diplomacy", 
          digits=1, 
...
table_us_dip_comp<-
  us_dip_c_y %>%
  filter(dip_rep_m==0 & dip_rep_a==0 & evt_n_c_y==0) %>% #Note that the variable dip_rep_m=0 for South Vietnam, which is inaccurate
  arrange(desc(n_c_y)) %>%
  top_n(n = 20, wt = n_c_y) 
...
stargazer(table_us_dip_comp[c("year",
          ...
          summary = FALSE,
          rownames = FALSE,
          title="Comparison of Variables on U.S. Diplomacy", 
          digits=1, 
