...
load("./data/cables_docs.RData")
#cables_docs<-docs
load("./data/frus_docs.RData")
...
classification_doc_month <- 
  tbl(cables_db,'classification_doc') %>%
  collect() %>%
  distinct() %>%
  mutate(
    date=as_date(date), 
...
ggplot(classification_doc_month, aes(month)) + 
...
classification_doc <- tbl(cables_db,'classification_doc') %>%
  collect() %>%
  distinct() %>%
...
stargazer(classification_doc[c("classification","n", "rel.freq")],
          summary = FALSE,
          rownames = FALSE,
          type = "text", 
...
#Note that the differences in total ns for each dataset.
```
...
sort(cable_n_all_states$cow_statename[cable_n_all_states$cow_statename %in% setdiff(cable_n_all_states$cow_statename, cable_n_states_70s$cow_statename)])

table(cable_n_all_states_year$cow_statename[cable_n_all_states_year$cow_statename %in% setdiff(cable_n_all_states_year$cow_statename, cable_n_states_70s_year$cow_statename)])
```
