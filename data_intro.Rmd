...
#Create a folder named "data_comparison" in the workding directory
```
...
#Most of the items with a discrepancy between country ids and iso-3166 numeric seem to be non-state entities, except East and West Germany.
...
#Download, save, or load the tables for tags and docs (doc_id and date) in the working directory
```{r}
tags<-
  tbl(mydb, 'tags') %>% 
...
us_dip_vis<-
  ...
  select(cowid, year, bi_PRE, bi_SOS, mil_ratio, USmilaid, allies, USdefense, USdefense_EUR, UStrade, energypc, USalign, UNpart, mpower) %>%
