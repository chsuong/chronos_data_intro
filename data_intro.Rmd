...
getwd()
setwd("/Users/clarahsuong/Dropbox/nyu_postdoc/ner/dataset_intro") #Set the working directory in the location where you can save large data files (file size limitation at Github)
...
#External datasets:
* Download the following datasets in the folder "data_comparison"
...
cow<-read_csv("./data_comparison/cow/COW country codes.csv") %>%
  distinct()
...
#                   "date")) %>%
#  tally() 
#save(cable_n_country_date, file = "cable_n_country_date.RData")
load("cable_n_country_date.RData")
```

```{r echo=FALSE}
#Examine cables tagged China, North Korea, and Vietnam as a test
...
```{r results="hide"}
us_dip_rep<-
  read_csv("./data_comparison/moyeretal2016/Pardee Center Diplomatic Representation_COW 20190208.csv", locale = locale(encoding = "UTF-8")) %>% #COW-compatible version
#  read_excel("./data_comparison/moyeretal2016/Diplomatic_Exchange_V3.16.16.xlsx") %>% #Non-compatible, public version
...
#write.csv(read.dta("diplomatic_core.replication.dta"),"/Users/clarahsuong/Dropbox/nyu_postdoc/ner/dataset_intro/data_comparison/lebovic_saunders_2016/diplomatic_core.replication.csv")
...
us_dip_vis<-
  ...
  dplyr::select(cowid, year, bi_PRE, bi_SOS, mil_ratio, USmilaid, allies, USdefense, USdefense_EUR, UStrade, energypc, USalign, UNpart, mpower, term2, TRU, EIS, KEN, JOH, NIX, FOR, CAR, demo, bi_L1_PRE, EUR, AFR, MEA, SCA, EAP, bi_L14_SOS, bi_L14_PRE, bi_L58_SOS, bi_L58_PRE) %>%
...
summary(glm.nb_pre1 <- glm.nb(y_n~bi_PRE, data = cable_n_vis_country_y))
summary(glm.nb_sos1 <- glm.nb(y_n~bi_SOS, data = cable_n_vis_country_y))

#Modified from the analysis in Lebovic and Saunders (2016)' Tables 3 and 4
summary(glm.nb_pre2 <- glm.nb(y_n~bi_PRE+mil_ratio+USmilaid+allies+USdefense+USdefense_EUR+UStrade+energypc+USalign+UNpart+mpower+term2+TRU+EIS+KEN+JOH+NIX+FOR+CAR+demo+bi_L1_PRE+EUR+AFR+MEA+SCA+EAP, data = cable_n_vis_country_y))
#Cluster by cowid

summary(glm.nb_sos2 <- glm.nb(y_n~bi_SOS+mil_ratio+USmilaid+allies+USdefense+USdefense_EUR+UStrade+energypc+USalign+UNpart+mpower+term2+TRU+EIS+KEN+JOH+NIX+FOR+CAR+demo+bi_L1_PRE+EUR+AFR+MEA+SCA+EAP, data = cable_n_vis_country_y))
#Cluster by cowid

summary(glm.nb_pre3 <- glm.nb(y_n~bi_PRE+mil_ratio+USmilaid+allies+USdefense+USdefense_EUR+UStrade+energypc+USalign+UNpart+mpower+term2+TRU+EIS+KEN+JOH+NIX+FOR+CAR+demo+bi_L14_SOS+bi_L14_PRE+bi_L58_SOS+bi_L58_PRE+EUR+AFR+MEA+SCA+EAP, data = cable_n_vis_country_y))
#Cluster by cowid

summary(glm.nb_pre3 <- glm.nb(y_n~bi_SOS+mil_ratio+USmilaid+allies+USdefense+USdefense_EUR+UStrade+energypc+USalign+UNpart+mpower+term2+TRU+EIS+KEN+JOH+NIX+FOR+CAR+demo+bi_L14_SOS+bi_L14_PRE+bi_L58_SOS+bi_L58_PRE+EUR+AFR+MEA+SCA+EAP, data = cable_n_vis_country_y))
#Cluster by cowid

#Check model assumptions and robustness (https://stats.idre.ucla.edu/r/dae/negative-binomial-regression/)

#Descriptive statistics
stargazer(as.data.frame(cable_n_vis_country_y), type = "text", 
          title="Descriptive statistics", 
          digits=1, 
          out="desc_country_year.txt"#,
